...
- **tools** ([tools-mcp](https://github.com/markus7h/tools-mcp)) wird aus `stdio.repo` geklont, mit `npm` gebaut und als stdio-MCP mit `TOOLS_MCP_REGISTRY_URL=stdio.registry_url` registriert. Das benötigt **node, npm und git** auf dem Client — fehlt etwas, gibt das Setup einen Installationshinweis aus und überspringt `tools` (alles andere läuft trotzdem durch).
